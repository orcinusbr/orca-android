...
final class ErrorDelegate {
   ...
   */
  ErrorDelegate(@NonNull final CompositionTextField textField) throws NoSuchFieldException {
   ...
   */
  void invalidate(@Nullable final Configuration configuration) {
   ...
   *     state.
   */
  void toggle(@Nullable final CharSequence error) {
    if (this.error != error) {
      this.error = error;
      if (error != null) {
        showWhenLaidOut();
  ...
  private TextPaint getTextPaint(final TextView textView) throws NoSuchFieldException {
   ...
   */
  private boolean didUIModeChange(@Nullable final Configuration configuration) {
   ...
   */
  private void animateTextFieldHeight(final IntUnaryOperator height) {
    final int currentHeight = textField.getHeight();
    final int targetHeight = height.applyAsInt(currentHeight);
    final ValueAnimator heightAnimator =
        ValueAnimator.ofInt(currentHeight, targetHeight).setDuration(256);
    heightAnimator.addUpdateListener(
        updatedHeightAnimator ->
            ViewKt.updateLayoutParams(
                textField,
                layoutParams -> {
                  layoutParams.height = (int) updatedHeightAnimator.getAnimatedValue();
                  return Unit.INSTANCE;
                }));
