...
/**
 * Converts a [Composable] into a [Drawable] and returns a [State] containing it.
 *
 * @param composable [Composable] to be made drawable.
 */
@Composable
fun drawableStateOf(composable: @Composable () -> Unit): State<Drawable?> {
  val drawableAsState = remember(composable) { mutableStateOf<Drawable?>(null) }

  AndroidView(::ComposeView, Modifier.drawWithContent {}) { view ->
    view.setContent(composable)
    view.addOnLayoutChangeListener { laidOutView, _, _, _, _, _, _, _, _ ->
